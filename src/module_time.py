...
# rewrite by timeLM
from models.modeling_phi_time import *
from models.configuration_phi_time import *

# Initializing a Phi-1 style configuration
# configuration = PhiConfig.from_pretrained("microsoft/phi-2", trust_remote_code=True)
...
class SFTModule(pl.LightningModule):
    def __init__(
        ...
        if load_pretrained:
            self.model = PhiForCausalLM(PhiConfig.from_pretrained(model_name))
            # self.model = PhiModel(PhiConfig.from_pretrained(model_name, trust_remote_code=True))
