        ...
        for example in self.data:
            ...
            if self.add_eos_token:
                ...
                if 'timestamp' in example:
                    example_time = len(example_tokens)*[example['timestamp'][:4]]
                else:
                    example_time = len(example_tokens)*[0]
            if self.concat_samples:
                tokens.extend(example_tokens)
                mask.extend(example_mask)
                times.extend(example_time)
            else:
...
class TextDataset(Dataset):
    def __init__(self, tokens, mask, times, seq_len:int=512, concat_samples:bool=True, randomize_max_position:int=-1):
        self.tokens = tokens
        self.mask = mask
        self.times = times
    ...
    def from_processor(cls, processor, seq_len:int=512, randomize_max_position:int=-1):
        return cls(processor.tokens, processor.mask, processor.times, seq_len=seq_len, concat_samples=processor.concat_samples, randomize_max_position=randomize_max_position)
...
def collate_fn(batch, left_pad=False, pad_token_id:int=0):
    ...
    # time_ids = torch.ones((len(batch), max_len), dtype=torch.long) * pad_token_id
...
class SFTDataModule(pl.LightningDataModule):
    def __init__(
        ...
        batch_size:int=8,
        seq_len:int=1024,
        tokenizer_name:str="EleutherAI/pythia-1b-deduped",
        concat_samples:bool=True,
    ...
    def setup(self, stage: str):
        ...
        if stage == "fit":
            ...
            train_file = os.path.join(self.data_dir, "rest.jsonl")
            with open(train_file, "rt") as f:
                train += [json.loads(l) for l in f]
        ...
        if stage == "predict":
            with open(os.path.join(self.data_dir, "old_qa_time_test.jsonl"), "rt") as f:
