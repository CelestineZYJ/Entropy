...
test_list = read_json_file_to_dict_list('/shared/nas2/yujiz/fic_time_lm/data/extract_test.json')
...
phimodel = GPTNeoXForCausalLM.from_pretrained("EleutherAI/pythia-410m-deduped")
phitokenizer = AutoTokenizer.from_pretrained("EleutherAI/pythia-410m-deduped")
checkpoint = torch.load('/shared/nas2/yujiz/fic_lightning/pythia-410m-plustimeorder/lightning_logs/version_2/checkpoints/epoch=146-step=8085.ckpt',map_location=torch.device('cpu'))
# for key in checkpoint.keys():
#     checkpoint[key] = checkpoint[key].to(dtype=torch.float16)
modified_checkpoint = {key.replace('model.', '', 1): value for key, value in checkpoint['state_dict'].items()}
phimodel.load_state_dict(modified_checkpoint, strict=True)
phitokenizer.pad_token = phitokenizer.eos_token
phimodel = phimodel.to(device)
...
k=100
new_test_list = [{'query':'Instruct:When did this event happen? President Donald Trump declares a national emergency to secure funding for a border wall on the US-Mexico border. Output:', 'text':'On February 15th, 2019'},
                  {'query':'Instruct:The state of Michigan enacted a ban on the sale of flavored vaping products. Output:', 'text':'On September 4th, 2019', 'predict_time':'x'},
                  {'query':'Chris Christie delivers a keynote speech at the Republican National Convention', 'text':'On August 28th, 2012', 'predict_time':'x'},
                  {'query':'Instruct:When did this event happen? Chris Christie delivers a keynote speech at the Republican National Convention. Output:', 'text':'On August 28th, 2012', 'predict_time':'x'},
                  {'query':'Instruct:When did this event happen? Taylor Swift wins Album of the Year at the Annual Grammy Awards for her album \"1989\". Output:', 'text':'On February 15th, 2016','predict_time':'x'},
                  {'query':'Instruct:When did this event happen? Widespread destruction and thousands of casualties in Sichuan earthquake. Output:', 'text':'On May 12th, 2008','predict_time':'On May 12th, 2008'},
...
for each_test_sample in tqdm(test_list):
    ...
    if 1:
        ...
        generated_answer = phitokenizer.decode(predict_time[0], skip_special_tokens=False)
        ...
        for layer in range(0, 23):
          # output_hidden_states["hidden_states"]: tuple of 32+1 (number of layers) of tensors of batch_size=1,seq_length=x,feature_dim=2560
          # adopt input sentence's hidden layer i's last token hidden 
          # state's 2560 vector
          hs_out = output_hidden_states["hidden_states"][layer+1][0,-4,:]
          # adopt input sentence's hidden layer i's second token 'When''s hidden state's 2560 vector
          # hs_out = output_hidden_states["hidden_states"][layer+1][0,1,:]
          
          # token: earthquake
          # hs_out = output_hidden_states["hidden_states"][layer+1][0,19,:]
          
          # print(decode_tokens(phitokenizer, hs_out.logit.detach().numpy().argmax(dim=1)))
          
          #proj = hs_out.matmul(E.T)
          # obtain the logits of hidden layer i's last token's hidden state
          proj = phimodel.embed_out(hs_out)
          proj = torch.softmax(proj, dim=0)
          # proj = torch.exp(-proj)
          proj = proj.detach().cpu().numpy()
          ind = np.argsort(-proj, axis=-1) 
          ...
          all_layer_time_token_probabi['top_k_preds_layer_'+str(layer)] = top_k_preds
          print('\n'+'$'*150)
          print('layer_'+str(layer)+': top_tokens: '+str(top_k_preds))
          
          top_str = ''
          for i in range(0,30):
            top_tok_score = proj[ind[i]]
            top_str+='/'+str(top_tok_score)
          print('layer_'+str(layer)+': '+top_str)
          
          # print('@'*100+'\npredict_token_probability')
          # predict_str = ''
          ...
          print('$'*100+'\nlabel_token_probability')
          # gene_tokens = generated_answer[:generated_answer.find('<|endoftext|>')].split(' ')[-2:] 
          # gene_tokens = [' ' + i for i in gene_tokens]
          # label_time_token_id = phitokenizer.convert_tokens_to_ids(gene_tokens)
          # label_token_idx = label_time_token_id
          
          # predict_token_idx = phitokenizer(generated_answer[generated_answer.find('<|endoftext|>'):], return_tensors='pt', padding=True)['input_ids']
          # predict_token_idx = predict_token_idx.numpy()[0]
          
          label_token_idx = phitokenizer.encode(output_label, add_special_tokens=True)
          # label_token_idx = phitokenizer(output_label+' 2088', return_tensors='pt', padding=True)['input_ids']
          # label_token_idx = label_token_idx.numpy()[0]
          ...
          print('$'*100+'\npredict_token_probability')
          predict_token_idx = phitokenizer.encode(generated_answer, add_special_tokens=True)
          label_str = ''
          time_tokens_prob = []
          for attribute_tok in predict_token_idx[-4:]:
            attribute_tok_rank = np.where(ind == attribute_tok)[0][0]
            attribute_tok_score = proj[ind[attribute_tok_rank]]
            attribute_tok_score = round(attribute_tok_score, 6)
            label_time_token = decode_tokens(phitokenizer, [attribute_tok])[0]
